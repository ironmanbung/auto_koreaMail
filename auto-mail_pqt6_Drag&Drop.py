...
class MailAutomationApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        scroll_content.setMinimumWidth(960)
        ...
        # ── 데이터 테이블 ─────────────────────────────────────
        # [요구추가1] Interactive 모드: 사용자가 마우스 드래그로 컬럼 너비 조정 가능
        self.table = QTableWidget(0, 0)
        hdr = self.table.horizontalHeader()
        hdr.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)   # ← 변경
        hdr.setMinimumSectionSize(30)
        hdr.setDefaultSectionSize(120)
        hdr.setStretchLastSection(True)                                # 마지막 컬럼은 남은 공간 채움
        ...
        diy_top = QHBoxLayout()
        diy_help = QLabel(
            "   ※ 위 버튼들을 드래그한 다음, 아래 슬롯에 놓으세요.  |  슬롯 간 이동, 빼기(더블클릭 또는 빈공간 이동 시) 가능합니다.  |  전체 슬롯 순서대로 실행됩니다"
        )
        ...
        # [요구추가2] on_changed 콜백으로 슬롯 변경 시 자동 저장
    ...
    def load_excel(self):
        ...
        try:
            ...
            # [요구추가1] 엑셀 로드 후에도 Interactive 모드 유지
            hdr = self.table.horizontalHeader()
    ...
    def _set_initial_column_widths(self, col_count):
        ...
        # 마지막 컬럼은 stretchLastSection이 처리하므로 나머지만 설정
        if col_count > 1:
    ...
    # ══════════════════════════════════════════════════════════
    #  설정 저장/불러오기
    #  [요구추가2] diy_slots 키를 추가하여 DIY 슬롯 상태를 JSON에 보존
    # ══════════════════════════════════════════════════════════
    ...
    def save_config(self):
        ...
        self.cfg.update({
            ...
            "diy_slots":  diy_data,   # ← [요구추가2] DIY 슬롯 상태 저장
        })
        try:
